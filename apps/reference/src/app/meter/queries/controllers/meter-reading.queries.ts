...
import { ReferenceGetIndividualMeterReadings, ReferenceGetMeterReadingsByUser, ReferenceGetMeterReadingsByHID } from '@myhome/contracts';
...
export class MeterReadingQueries {
    ...
    @RMQValidate()
    @RMQRoute(ReferenceGetMeterReadingsByUser.topic)
    async getMeterReadingsByUser(@Body() dto: ReferenceGetMeterReadingsByUser.Request) {
        try {
            return await this.meterReadingQueriesService.getMeterReadingsByUser(dto);
        } catch (e) {
            throw new RMQError(e.message, ERROR_TYPE.RMQ, e.status);
        }
    }
